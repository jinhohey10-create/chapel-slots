...
- 화면: 섹션 `#dress` 안에서 [견적 계산 / 샵별 보기 / 추가금 구조 / 시세] 네 뷰. 보던 뷰와 계산기 입력값은 `localStorage` 의 `dress-view`. 엑셀에 10~12 시트.
  - **견적 계산**: 묶음가 + 원본·수정본 44만 + 헬퍼비(촬영 25 / 본식 25, 야외면 +5) + 투어 피팅비 5.5만×샵수 + 라벨 업그레이드 + 2부 드레스. 계산 항목은 `calcLines()` 한 곳에만 있고 화면·엑셀이 같이 쓴다.
- **드레스샵 21곳 전부 홈페이지가 없다. 공식 채널이 인스타그램뿐이다** — 20곳의 핸들을 `data/dress.json` 의 `ig` 에 넣고 샵별 보기에서 링크로 건다. 가격표를 올리는 곳도 대개 인스타 하이라이트라 로그인 없이는 못 본다.
- instagram.com 은 열리긴 하나 **429 로 거의 막힌다**(어쩌다 한 번 200). 프로필 HTML 에도 가격은 없다. 다시 시도해 볼 가치가 낮으니 사용자 캡처를 받는 편이 빠르다. threads.com · price.go.kr 은 아예 안 열린다.
